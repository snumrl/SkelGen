...
int main(){
	...
	vector<TiXmlElement*> muscleElemVec;
	int idx = 0;
	for(TiXmlElement* muscle = muscledoc->FirstChildElement("Unit");muscle!=nullptr;muscle = muscle->NextSiblingElement("Unit"))
	{
		...
		for (TiXmlElement* waypoint = muscle->FirstChildElement("Waypoint"); waypoint!=nullptr; waypoint = waypoint->NextSiblingElement("Waypoint")) {
			...
			TiXmlElement* wp_elem = new TiXmlElement("Waypoint");
			wp_elem->SetAttribute("body", waypoint->Attribute("body"));
			wp_elem->SetAttribute("p", waypoint->Attribute("p"));
			wp_elem->SetAttribute("idx", str);
			indices = indices + str + " ";
			outputMuscleDoc->LinkEndChild(wp_elem);
		}

		TiXmlElement* muscle_elem = new TiXmlElement("Unit");
		muscle_elem->SetAttribute("name", muscle->Attribute("name"));
		muscle_elem->SetAttribute("f0", muscle->Attribute("f0"));
		muscle_elem->SetAttribute("lm", muscle->Attribute("lm"));
		muscle_elem->SetAttribute("lt", muscle->Attribute("lt"));
		muscle_elem->SetAttribute("pen_angle", muscle->Attribute("pen_angle"));
		muscle_elem->SetAttribute("indices",indices);

		muscleElemVec.push_back(muscle_elem);
		//outputMuscleDoc->LinkEndChild(muscle_elem);
	}
	for (auto& muscleElem: muscleElemVec) outputMuscleDoc->LinkEndChild(muscleElem);
